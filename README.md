...
内置了当前年份一份 `data/2026.json` 作兜底，来源 <https://github.com/NateScarlet/holiday-cn>，该仓库跟随国务院办公厅通知更新。**不需要每年手动改文件**，应用会自己联网补齐：

| 时机 | 行为 |
|---|---|
| 每次启动、回到前台 | 检查今年的数据；10 月起同时检查明年的 |
| 某年还没有数据 | 每天最多联网查一次，直到拿到为止 |
| 某年已有数据 | 每 30 天复查一次，官方偶有修订 |
| 用户点「立即获取」 | 忽略间隔立刻查，并用提示告知结果 |

下载到的数据存在本机（键 `holiday-data-v1`），下次启动直接装入，优先级高于内置文件。三个下载地址依次尝试，前两个是 jsDelivr 的国内可达镜像，最后是 GitHub 原始地址。下载内容会做格式校验，年份或字段不对一律丢弃，不会污染日历。

界面上「尚无 XXXX 年节假日数据」的提示只会在联网也拿不到时出现，通常是国务院还没发布。内置文件只是兜底，每次发新版本顺手把当年的换成新一年的即可，不换也不影响使用。

相关代码：`lib/holidaySource.ts` 是来源与校验规则，`lib/holidayStore.ts` 负责下载与本地缓存，`composables/useHolidays.ts` 负责触发和通知页面刷新。

## 六、数据存储与备份

打卡记录通过 `uni.setStorageSync` 存在本机应用自己的存储区，键为 `punch-records-v1`，格式：

```json
{ "2026-09-21": { "checkIn": 1789999800000, "checkOut": 1790037600000 } }
```

App 端底层是原生持久化存储（不是浏览器缓存），不会被系统自动清理。什么情况下会丢：

| 操作 | 数据 |
|---|---|
| 覆盖安装新版本（蒲公英装同一 Bundle ID 的新包） | 保留 |
| 卸载再装 | 丢失 |
| 换手机 | 不会自动带过去 |
| iCloud 或电脑整机备份再恢复 | 保留 |
| 真机调试基座里产生的数据 | 存在基座里，正式包看不到 |

主界面底部「数据备份与恢复」页提供：

- **复制到剪贴板**：整份记录变成 JSON 文本，粘贴到备忘录或微信文件传输助手即可保存。
- **保存为文件**（App 端）：写到应用文档目录并打开系统预览，从预览页可存到「文件」App 或分享出去。H5 端为直接下载。
- **从剪贴板导入** / **粘贴导入**：解析后可选「合并」（同一天以导入为准，其余保留）或「替换」（清空后导入）。格式不对的条目会跳过并提示数量。

备份格式在 `lib/backup.ts`，也接受直接粘贴存储里的原始对象。换手机或重装前先导出一份。
